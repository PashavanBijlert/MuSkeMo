...
Wrapping in Gaitsym muscles is currently ignored during import, but this will be added in a future update. Wrapping cylinders have infinite height in Gaitsym, which is not possible in MuSkeMo. MuSkeMo allows you to set a default height during import, and you can change the height on a per-cylinder basis after importing (see sec. \ref{sec:wrapgeom}).

Visual geometry can be imported, but requires the user to type the name of the containing folder in "Gaitsym geometry folder". The geometries must be in a subdirectory of the model directory, and the name of this subdirectory must currently be manually typed into the panel.
