...
It is possible to change the density property of an object, after which you'll have to select the object and rerun "Compute for selected meshes". See \ref{sec:inpropvalidation} for a validation of the outputs.
...
% vol_skin = 10^(intercept)*vol_CH^slope * 10^(MSE/2)
% y = 10**a * x**b * 10**(MSE/2)

%Brassey 2016 pigeon equations have mm3 as input and g as output. To convert to kilograms (multiply output by 10**-3) and meters**3 (multiply the input by 10*9), use:
% a_new = a -3 + 9*b
