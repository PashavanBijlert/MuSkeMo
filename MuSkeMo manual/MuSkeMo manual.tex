...
\subsection{What is a MuSkeMo model?}
\label{sec:muskemomodel}
MuSkeMo is intended to construct, visualize, and analyze musculoskeletal models that can be used in multibody dynamic simulators, such as OpenSim \cite{sethOpenSimSimulatingMusculoskeletal2018}. In its most basic form, this consists of a chain of massless rigid bodies connected by joints, which is in principle enough to perform kinematic analysis, which can be performed in Blender itself using MuSkeMo. For dynamic simulations (outside Blender), it is usually necessary to define inertial properties of the rigid bodies, assign contact points, and add (muscle) actuators.

Models do not need to form a single chain, they can have a tree structure (e.g., a torso segment with four legs attached to it). The only limitation on model topology is that there can be no closed kinematic loops -- this means that in MuSkeMo, a rigid BODY (sec. \ref{sec:bodypanel}) can be the parent body of multiple JOINTs (sec. \ref{sec:jointpanel}), but it can only be the child body of one JOINT. This is to ensure compatibility with dynamic simulator, where closed kinematic loops can be approximated using kinematic constraints. Kinematic constraints are not yet officially supported in MuSkeMo, but advanced users may be able to use Blender's inbuilt functionality to achieve the desired behavior.
...
Constructing a model in Blender provides you with a lot of freedom to interactively repose model elements during model construction. However, many of the computations are only valid in the specific pose they are computed in. For example, when computing mesh inertial properties using the inertial properties panel (\ref{sec:inproppanel}), the inertial properties in the global reference frame are only valid as long as the orientation and position (the pose) of the mesh do not change. Similarly, object positions and orientations with respect to parent frames (\ref{sec:frame}) are only valid during export if their relative positions and orientations remain unchanged. 

To prevent incorrect exports, MuSkeMo keeps track of the `default\_pose', which is the pose in which the relevant computation was performed (during model construction or import). If you try to perform an operation that is only valid in the default pose, the operation is cancelled (with an error message) if the object is not in its default pose. In that situation, you must reposition the model components back into the default pose before exporting. MuSkeMo provides a button for this: ``Reset to default pose", available from the Global settings panel (sec. \ref{sec:MSM_glob_settings}), the Joint panel (sec. \ref{sec:jointpanel}), the Body panel (sec. \ref{sec:bodypanel}), the inertial properties panel (sec. \ref{sec:inproppanel}), and the export panel (sec. \ref{sec:exportpanel}). The button works by selecting one or multiple objects that have a default pose, and then pressing the button. If a single object is selected and it is part of a model, MuSkeMo traverses the model until it finds the root joint or body (i.e., the component at the top of the parent hierarchy), and then resets all the joints and bodies to their default pose. If multiple components of the same model are selected, MuSkeMo only performs the operation once (because they will both have the same root). If multiple disjointed objects are selected, MuSkeMo will reset the default pose for all the separate sections (including individual meshes with inertial properties).

Alternatively, if you wish to export the model in the changed pose, it is necessary to recompute all the model parameters in this new pose. For inertial properties and bodies, this means recomputing and reassigning the inertial properties, for other components it means detaching and reparenting the objects.

Default pose checks use Numpy's \href{https://numpy.org/doc/stable/reference/generated/numpy.allclose.html}{allclose} function. Under exceptional circumstances, the default settings for the default pose checks are too restrictive. For example, accumulated single-precision rounding errors (see sec. \ref{sec:precision}) may occur if you have a long chain of segments which each have a large change in orientation with respect to the parent. This could lead to false positives -- MuSkeMo raises the default pose error even though you are positive that the model components are in fact in their default pose. In this situation, you can (at your own peril) raise the tolerances for the default pose checks in the global settings panel (sec. \ref{sec:MSM_glob_settings}). A possibility is to raise the absolute tolerance to 5e-5. Note that repeated default pose errors may also indicate some underlying model issue, especially if raising absolute tolerance to 5e-5 did not help.

Conversely, the absolute tolerance in the default pose checks may actually be too permissive if you are constructing extremely small (micron-scale) models. See the \href{https://numpy.org/doc/stable/reference/generated/numpy.allclose.html}{allclose} documentation to make an informed decision if you intend to construct tiny models.
...
The global settings panel has a subpanel named ``Default pose tolerances'', where you can change the absolute and relative tolerances for default pose enforcement (see sec. \ref{sec:defaultpose}).
...
\subsubsection{Geometric primitive fitting}
\label{sec:primitivefitting}
In this panel, there are buttons that fit geometric primitives to a selected mesh (sphere, cylinder, ellipsoid, plane). Simply select a target mesh, and press one of the fitting buttons. The positions and/or orientations of the fitted objects can be transferred to joints (sec. \ref{sec:jointutilities}). The options are:
...
\begin{itemize}
    ...
    \item \textbf{Macaulay 2023 Average (Bird and Non-Avian Sauropsid):} The per-segment ``Av." average expansions from Macaulay et al. 2023 \cite{macaulayDecouplingBodyShape2023}, supplementary data S6. This uses the average expansions, except for the tail which is based solely on non-avian sauropsids.
    \item \textbf{Macaulay 2023 Mixed (Bird and Non-Avian Sauropsid):} A custom variant based on the data from Macaulay et al. 2023 \cite{macaulayDecouplingBodyShape2023}. This uses the average expansions, except for the tail (based solely on non-avian sauropsids), and the neck, shank, metatarsus, and pes segments (based solely on birds).
...
Multiple joints can use the same parent body, but bodies can only be the child of one joint, to prevent closed kinematic loops \ref{sec:muskemomodel}.
...
\label{sec:jointutilities}
\paragraph {Fitting geometric primitives for joint centers.} The geometry primitive fitters (see sec. \ref{sec:primitivefitting}) from the Mesh tools panel are also made available here, and fitted positions and orientations can be used for joints.

\paragraph {Matching joints to fitted geometry.} 

It is possible to match the position and/or orientation of a joint to the fitted geometry, respectively. Simply select the target joint and a source geometry, and press the respective button. Child objects and parent objects are not transformed with the joint. Instead, only the joint's position or orientation is changed, and related data (e.g., pos\_in\_child) are recomputed automatically. 
...
\subsubsection{Create and assign wrapping}
\label{sec:createassignwrapping}
You can use the ``Wrapping" subpanel to perform all the necessary steps to create and assign wrapping to a muscle. This includes creating WRAP object geometry (cylinders, spheres, and ellipsoids, sec. \ref{sec:wrapgeom}), assigning parent bodies, and assigning muscles to wrap around WRAP geometry objects. The panel also performs the reverse operations. See \ref{sec:musclewrapping} for details on how the wrapping works, what the limitations are, and what settings are available. Note that wrapping is currently only physically accurate (i.e., a true tangent solution) if each wrapped section is separated by at least one muscle point on both sides.

It is possible to automatically create WRAP objects from 1+ compatible fitted GEOM\_PRIMITIVES (sec. \ref{sec:primitivefitting}). This is currently possible for spheres, ellipsoids, and cylinders.
...
MuSkeMo currently supports cylindric, spherical, and ellipsoidal wrapping. MuSkeMo adds a Geometry nodes modifier to the muscle for each target wrapping object, which has to be a WRAP object (sec. \ref{sec:wrapgeom}). For clarity: the geometry node modifier on the WRAP determines the dimensions of the geometry (e.g., cylinder radius), whereas the geometry node modifier on the MUSCLE determines how the wrapper should behave. This allows multiple muscles to use the same target WRAP geom, while having individualised behaviors (e.g., wrapping about different sides of the same cylinder). Both WRAP creation and assignment are performed in the Wrapping subpanel of the Muscle panel, see sec. \ref{sec:createassignwrapping}.

It is important to be mindful of the following limitations:

\begin{itemize}
\item \textbf{If you add multiple wrapping objects per pair of muscle points, MuSkeMo only provides true tangent curve solutions if each wrapped section is separated by a curve point.} Typically, wrapping around arbitrary objects requires iteratively recomputing the wrap until the solution has stabilized (e.g., \cite{garnerObstacleSetMethodRepresenting2000,scholzFastMultiobstacleMuscle2016}), and this is currently challenging to implement in geometry nodes.
\item \textbf{Wrapping will not work correctly if any of the muscle curve points clip into the wrapping object.} Wrapping is mathematically not defined if a muscle point is inside the wrapping object, it is your responsibility during model construction to prevent this.
\item The sphere and ellipsoid wrappers are currently \textbf{experimental}, I have not yet performed a thorough comparison or test. If you plan to use it for anything else than visualization, perform thorough test that the expected lengths and/or moment-arms are sensible.
\end{itemize}


\paragraph{Cylinder and sphere.}
MuSkeMo implements \cite{garnerObstacleSetMethodRepresenting2000} for cylindric and spherical wrapping, using a custom-designed Geometry node. These wraps are implemented as true tangent-curve, minimum distance solutions (true geodesics). Lengths of the wrapped paths are computed using expressions for the exact path lengths as derived in \cite{garnerObstacleSetMethodRepresenting2000}. The wrapper node visualizes the wrapped section by using a \href{https://docs.blender.org/manual/en/latest/modeling/geometry_nodes/geometry/sample/raycast.html}{Raycast node} to project intermediate points onto the surface between the two tangent points.

\paragraph{Ellipsoid.}
\label{sec:ellipsoidwrapping}
Ellipsoidal wrapping typically requires some type of iterative approach (e.g., \cite{scholzFastMultiobstacleMuscle2016}), which is challenging to implement in Geometry Nodes. MuSkeMo instead implements ellipsoidal wrapping by scaling the ellipsoid to a unit sphere, applying standard spherical wrapping logic, and then scaling back to the ellipsoidal coordinates. Geodesic wrapping around spheres is always on a plane through the sphere origin -- this plane called a ``great circle'' of the sphere, which splits the sphere in two equal halves. Applying a similar wrap to an ellipsoid results in a wrap on a plane through the ellipsoid origin, equivalently called a ``great ellipse'', and it also splits the ellipsoid in two equal halves. \textbf{Importantly, because MuSkeMo's ellipsoidal wraps are not true geodesics, the wrapped length will in general be slightly longer than a true minimum-length ellipsoid wrap}. For visualization, this is unlikely to be noticeable, but when computing moment arms and/or muscle lengths, be aware that although MuSkeMo reports the muscle path length as it is being visualised, that path is not the shortest possible path across the ellipsoid. The benefit of this simple approach is that it behaves as smoothly and is as computationally efficient as the standard sphere wrapper.

OpenSim users should be aware that OpenSim's current ellipsoid wrapping algorithm (which has `midpoint', `axial', and `hybrid' variants) is also not a true geodesic (see the comparison in \cite{marsdenNovelApproachPrediction2008}). OpenSim's implementation will give different results than MuSkeMo's approach, although the differences are likely to be small (a comparison will be provided in the future).


The wrapper node visualizes the wrapped section by using a \href{https://docs.blender.org/manual/en/latest/modeling/geometry_nodes/geometry/sample/raycast.html}{Raycast node} to project intermediate points onto the surface between the two tangent points. Because the ellipsoid wrapper is not an exact geodesic, the length of the wrapped curve section is computed by summing the individual curve sections projected onto the ellipsoid surface. This is sensitive to the ``Wrapping point resolution'' setting (see below), and to the ``Ellipsoid resolution'' setting (see sec. \ref{sec:wrapgeom}).

\paragraph{Wrap settings.}

When a WRAP is assigned to a muscle, it receives a very complex geometry nodes modifier. Changing the WRAP geometry dimensions (e.g., the cylinder's radius) also affects the dimensions of the muscle wrapping, this information is carried over via \href{https://docs.blender.org/manual/en/latest/modeling/geometry_nodes/geometry/read/named_attribute.html}{Named Attributes} in the Geometry node for the Wrapping object, which is then sampled in the Geometry node in the muscle wrapping node.

Wrapper nodes have the following settings, accessible via the wrapping modifier on a muscle (after wrap assignment):
...
In MuSkeMo, it is possible to fit geometric primitive shapes (spheres, cylinders, ellipsoids, and planes) to 3D meshes, see \ref{sec:primitivefitting}. This can be useful for defining joint centers of rotation using the skeletal geometry (sec. \ref{sec:jointutilities}), or for using as wrapping geometry. The geometric details of the shapes are stored for reuse. These are:
...
WRAP geometry are geometric primitives that act as wrapping targets for muscles. WRAP objects are not regular meshes like GEOM\_PRIMITIVES, they are parametrically generated using Geometry nodes. This allows you to alter the wrapping geometry dimensions (e.g., cylinder radius) via the modifier on the object (press the blue wrench in the interface, see sec. \ref{sec:modifiers}). For clarity: the geometry node modifier on the WRAP determines the dimensions of the geometry (e.g., cylinder radius), whereas the geometry node modifier on the MUSCLE determines how the wrapper should behave. This allows multiple muscles to use the same target WRAP geom, while having individualised behaviors (e.g., wrapping about different sides of the same cylinder). See sec. \ref{sec:musclepanel} and \ref{sec:musclewrapping} for details. 

Wrapping geometry can be created manually, or automatically from a GEOM\_PRIMITIVE, in the Wrapping subpanel of the Muscle panel (see sec. \ref{sec:createassignwrapping}). WRAPs are also automatically created and applied during model import if possible.

Currently, cylinders, spheres, and ellipsoids are supported. Ellipsoidal wrapping is not a true geodesic, but an approximation that is faster but results in slightly longer wrapped paths (see sec. \ref{sec:ellipsoidwrapping}). Spherical and ellipsoidal wrapping are both currently in experimental phase, use with caution.
...
\begin{itemize}
    ...
    \item \texttt{wrap\_type}. Currently can be `Cylinder', `Sphere', or `Ellipsoid'.
\end{itemize}

The wrapping geometry is created in a geometry node modifier, and the dimensions of the wrapping objects (e.g., cylinder radius) can be altered via the modifier input. The geometry node group stores those parameters as a \href{https://docs.blender.org/manual/en/latest/modeling/geometry_nodes/geometry/read/named_attribute.html}{Named Attribute} on the object. Once a wrapper node is assigned to a muscle, those named attributes are passed to the wrapper node.

The dimensions inputs differ per wrapping object, and are all in meters:

\begin{itemize}
\item \textbf{Radius:} Radius of the cylinder or sphere.
\item \textbf{Height:} Height of the cylinder.
\item \textbf{Radius x:} First radius of the ellipsoid.
\item \textbf{Radius y:} Second radius of the ellipsoid.
\item \textbf{Radius z:} Third radius of the ellipsoid.
\item \textbf{Ellipsoid resolution:} Resolution of the ellipsoid. Increase this when computing moment arms while using ellipsoid wrapping.
\end{itemize}
...
The following list of people have pointed out bugs, or suggested improvements to MuSkeMo, for which I am grateful:

\begin{itemize}
\item Zuhayr Parkar (early adopter)
\item Matt Dempsey (likely built more models with MuSkeMo than me at this point, gave several suggestions)
\item Max Herde (found several bugs)
\item Mark Semple (gave permission to adapt his ellipsoid fitter Python implementation)
\item Yuting Lin (found several bugs)
\item Harriet Nuttall (found a bug, and also prompted some improvements)
\item Lars D'Hondt (found several bugs)
\item Karl Bates (found several bugs, provided comments on the manuscript)
\item Anne Schulp (provided comments on the manuscript)
\item Oliver Demuth (suggested trying out a spherical transformation for the ellipsoid fitter)
\end{itemize}
