...
Bishop et al. \cite{bishopIntelligentSamplingHighdimensional2023} proposed an elegant stress test for pose sampling procedures. The test consists of a screw and a thread, with relatively little spacing between the two FIGURE. The screw has very little clearance to move through the thread, and requires a simultaneous rotation and translation to do so without intersections. The goal of the test is to find the simultaneous coupled rotations and translations of the screw as it passes through the thread.
The test is essentially designed to punish exhaustive search methods: if sampling at random, one has to perform millions of trials to even find a few viable points, and this is generally speaking too computationally costly (e.g., when using booleans \cite{manafzadehROMMappingLigamentous2018,demuth3DHindlimbJoint2020}). I have nevertheless performed the test using an exhaustive search approach, to demonstrate MuSkeMo's intersection checking speed, and because the recoved viable poses can be compared to an a priori known ideal result. However, exploring viable poses starting from a feasible point would recover substantially more viable points than a grid search approach \cite{bishopIntelligentSamplingHighdimensional2023}. 
...
The total search time is still long, and the sample density is still lower than Bishop et al.'s reference results \cite{bishopIntelligentSamplingHighdimensional2023}. This is because we are exhaustively searching through all combinations of poses, which is not an efficient approach. A search method that explores future viable poses starting from existing viable poses would dramatically improve performance \cite{bishopIntelligentSamplingHighdimensional2023}. Parallelization provided Bishop et al. a further benefit \cite{bishopIntelligentSamplingHighdimensional2023}, but this is currently not possible in Blender. However, because MuSkeMo's intersection checker is so fast, simple pose sampling analyses may not require it (e.g., sec. \ref{sec:posesampling}). Most biological analyses will not require sampling as many poses as the stress test.

The theoretical ideal translations and orientations plotted in FIGURE are derived in the supplementary information of \cite{bishopIntelligentSamplingHighdimensional2023}. The Matlab code that generates FIGURE is provided with the Blender scene on GITHUB LINK.
