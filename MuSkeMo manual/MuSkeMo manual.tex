...
\subsection{Photogrammetry Kinematics Toolbox}
\label{sec:pktoolbox}
The Photogrammetry Kinematics Toolbox is a collection of features aimed at acquiring kinematic and spatiotemporal (stride length) data using photogrammetry. This panel is currently under development, and will be merged into MuSkeMo once testing and validation has been completed.
\subsubsection{Setting up a scene}
The toolbox is meant to be used on photogrammetry data acquired from videos, specifically: a mesh or pointcloud reconstruction, combined with camera positions and orientations of each video frame. There are many programs that allow you to perform photogrammetry, and it is possible to import photogrammetric reconstructions into Blender using the \href{https://github.com/SBCV/Blender-Addon-Photogrammetry-Importer?tab=readme-ov-file}{Photogrammetry Importer} addon developed by Bullinger et al. \cite{bullingerPhotogrammetrybasedFrameworkFacilitate2021}.

\subsubsection{Workflow}
If using the Photogrammetry Importer addon, you can delete the individual cameras, select the ``animated camera'', right click, and hit ``set active camera''. Pressing the ``0'' key will now provide the undistorted camera-view, and pressing left and right on the keyboard will cycle through the frames in the timeline. The camera view is projected onto the mesh reconstruction of the environment. Using the overall direction of the animal with respect to its environment, combined with the assumption that its body was mostly vertically oriented in the frontal plane, we can construct (para-)sagittal plane onto which we can project the joint positions as viewed through the camera. This will provide us with 2D joint positions, with perspective distortions corrected using photogrammetry.

The steps are:

\begin{enumerate}
\item Fit a plane through the point cloud or mesh of the ground acquired using photogrammetry, using MuSkeMo's fitting tools (sec. \ref{sec:meshtools} and sec. \ref{sec:newmeshselportion}). This will be the ground plane.
\item While looking through the camera, place normal LANDMARKs (sec. \ref{sec:landmarkpanel}) on the ground plane corresponding to successive footfalls of the same foot (minimum 2). Blender's projections using the 3D cursor are not very accurate, so hit the button ``Set recommended snapping settings'', then manually replace the LANDMARKs by selecting them and pressing ``G''.
\item Create a sagittal PROJECTION\_PLANE using the button in the panel. This fits a plane through the two succesive footfalls, perpendicular to the ground. Ensure that you select the correct starting frame - it should correspond to initial contact of the first footfall.
If your plane is upside down (meaning Y is downwards with respect to the camera), undo this and flip the ground plane by 180 degrees about its local X or Y axes.
\item Create one or multiple ANIMATED\_LANDMARKs. Starting at initial contact, once again looking through the camera (press ``0''), sequentially place all the ANIMATED\_LANDMARKs in the desired positions, and press ``Add keyframe to animated landmarks''. Repeat this for each frame in the stride. The keyframe button can act on all ``ANIMATED\_LANDMARKS'' in the designated collection, or on the ones manually selected. Use the recommended snapping settings. Keyframes between non-sequential frames are automatically interpolated using linear interpolation, but this can cause slight projection inaccuracies (see sec. \ref{sec:projectiontolerance} on projection tolerance).
\item If desired, add as many strides to the sagittal plane, as long as they are in a different orientation and position as the previous ones. Each stride should start at the initial contact of a limb.
\item Export the ANIMATED\_LANDMARKS using the Export panel (sec. \ref{sec:exportpanel}). The landmark positions are expressed in the reference frame of the projection plane using eq. \ref{eq:localposition}. If the z-coordinate is not zero (meaning the point is not correctly projected onto the plane), the exporter warns the user and does not export the data.
\end{enumerate}

\subsubsection{Projection tolerance}
\label{sec:projectiontolerance}
Blender objects in the viewport are subject to single precision (see sec. \ref{sec:precision}). However, the precision of manual object placements is dependent on how far the viewport is zoomed in. This is relevant when projecting objects, such as when placing the 3D cursor (using shift + right click), or when positioning (animated) landmarks using the recommended snapping settings. To ensure projection accuracy, MuSkeMo throws an error if the distance from an object to the plane remains below the projection tolerance. By default, projection tolerance is set $l_{stride}/20000$ during sagittal plane creation, and this tolerance is maintained until export unless the user changes its value. For a stride length of 2 \unit{m}, this corresponds to a tolerance of 0.1 \unit{mm}, a tight tolerance to ensure that the projections do not introduce unnecessary errors. Note that the units in Blender are in meters by default, but if the photogrammetry model is not correctly scaled, could correspond to different real-world units.

If only a selection of the stride is keyframed (and the rest is interpolated automatically), Blender's finite precision can also result in some of the interpolated landmark positions exceeding the tolerance. In this situation, it is advisable to add a few additional keyframes to reduce the interpolation error. However, it is also possible to raise the projection tolerance, which will suppress the export error.
...
\subsection{Photogrammetry Kinematics Toolbox}
\label{sec:pktoolboxdatatypes}
The Photogrammetry Kinematics Toolbox adds several datatypes to MuSkeMo that behave differently from the normal datatypes, because they are intended to be animated using keyframes (see sec. \ref{sec:pktoolbox}).

\subsubsection{PROJECTION\_PLANE}
This is a (para)sagittal plane constructed from the footfall positions and a fitted ground plane (see sec. \ref{sec:pktoolbox}). It stores the following information:

\begin{itemize}
\item \texttt{current\_stride}. This keyframed property keeps track of strides. This property starts at 1 (keyframed at the frame number at which initial contact occurs), and increases in steps of 1 after each additional stride that is added.
\item \texttt{stride\_start\_frames}. This is a list with the frame numbers at which each sequential frame starts.
\end{itemize}

The underlying object type in Blender is a planar mesh.

\subsubsection{PROJECTION\_PLANE\_FRAME}

Unlike MuSkeMo's regular FRAMEs (sec. \ref{sec:frame}), PROJECTION\_PLANE\_FRAMEs are parented directly to a PROJECTION\_PLANE. This frame makes it easy to diagnose whether the projection plane is upside down (with y pointing downwards). These frames have a \texttt{parent\_plane}, which is the PROJECTION\_PLANE.

\subsubsection{ANIMATED\_LANDMARK}

Unlike MuSkeMo's regular LANDMARKs (sec. \ref{sec:landmark}), ANIMATED\_LANDMARKs are parented directly to a PROJECTION\_PLANE. The position of these landmarks is keyframed (while projecting onto plane), and during export, the 2D position with respect to the plane is exported (see sec. \ref{sec:pktoolbox}).
