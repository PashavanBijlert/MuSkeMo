...
\paragraph{Projection Angle} affects the wrap in two situations:

\begin{enumerate}
\item If Force Sided Wrap is on, we find the closest point $\vec{p}_c$ between the wrap object center $\vec{c}_w$, and the two points that span it ($\vec{p}_{1}$ and $\vec{p}_{2}$) using:
 \begin{equation}
...
\paragraph{Shortest wrap} determines $-\vec{p}_{1} \cross \vec{p}_{2}$, then computes the dot product between that vector and the cylinder z-axis. If this dot product is negative, the wrap is flipped. Shortest wrap is superceded by Force Sided Wrap.
