...
\item \textbf{FastOrVolumeAccurate: } The volumetric visualizer has two operation modes. The Fast mode may underestimate the volume by ~1-5\%, depending on the shape of the muscle, but it maintains its relative accuracy for profile curves shapes. The VolumeAccurate node adds an extra volume checking and rescaling step to ensure that the visualized volume is correct with respect to the muscle-input volume. This mode probably has lower performance, because it essentially draws each muscle twice (once internally).
\end{itemize}
