...
Construct anatomical and local reference frames, by assigning landmarks or markers as the reference points to construct the axes directions. The user defines a primary axis and a temporary axis direction, which together span a plane. The secondary axis is computed as perpendicular to the plane, and then the tertiary axis is mutually perpendicular to the first two axes. The reference frame is placed at the user-specified origin position.
