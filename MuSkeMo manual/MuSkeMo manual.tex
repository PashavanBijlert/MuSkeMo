...
\begin{itemize}
    ...
    \item \textbf{Plane.} Simple implementation based taking the covariance matrix of all the recentered vertex positions. The plane is taken as the basis spanned by the eigenvectors of the two largest eigenvalues. Right-handed coordinate systems are also ensured.
\end{itemize}
...
Similar to the simple muscle visualizations, volumetric muscle visualizations are also achieved by adding a Geometry node to each muscle. The node-group itself is shared across muscles, but muscle visualizations are individualized through four parameters and an operation mode, which can be accessed directly in the "VolumetricMuscleViz" modifier. These are:
