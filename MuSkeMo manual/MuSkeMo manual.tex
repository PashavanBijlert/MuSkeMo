...
%In essence, Custom Joints allow users to define custom rotation application order about arbitrary axes, and allows translations about arbitrary axes as well (and the translation and orientation axes do not need to match). Such flexibility is not currently possible in MuSkeMo, R1 is always coordinate_Rx (about the joint's local (1,0,0) axis), R2 is always coordinate_Ry, and R3 is always coordinate_Rz, and Tx, Ty, and Tz are about the same axes as the rotational axes. If a CustomJoint defines a different rotation order than Rx Ry Rz, and/or defines any of the transformations about different axes than (1,0,0), (0,1,0), or (0,0,1), then the transform axes are treated as "non standard". In this situation, MuSkeMo stores the name of the "rotation1" coordinate as the "coordinate_Rx" (regardless of whether it is an x-axis rotation), "rotation2" as "coordinate_Ry", and "rotation3" as "coordinate_Rz", and the same for the translatory coordinates. The custom transform axes as a Python dictionary in a custom property named "transform\_axes". These transform axes are mapped onto the joint rotations themselves when importing trajectories (see \ref{sec:trajectoryimport}), using axis-angle rotations (see sec. \ref{sec:axisanglerotation}). MuSkeMo warns the user during model import if such transform axes were detected.
%Non-standard axes are also assumed when the root joint has a non-zero rotation with respect to the global reference system in the default pose.
